...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $salary_type = filter_var($_POST['salary_type'], FILTER_SANITIZE_STRING);
    $salary_rate = filter_var($_POST['salary_rate'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
    ...
    try {
        ...
        if (!empty($position)) { $update_fields[] = "position = ?"; $params[] = $position; }
        if (!empty($salary_type)) { $update_fields[] = "salary_type = ?"; $params[] = $salary_type; }
        if (!empty($salary_rate)) { $update_fields[] = "salary_rate = ?"; $params[] = $salary_rate; }
...
<div class="bg-white p-6 rounded-lg shadow-md">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input class="shadow-sm appearance-none border rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" id="position" name="position" type="text" value="<?php echo htmlspecialchars($user['position']); ?>">
        </div>
        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2" for="salary_type">
                Salary Type
            </label>
            <select class="shadow-sm appearance-none border rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" id="salary_type" name="salary_type">
                <option value="monthly" <?php echo ($user['salary_type'] == 'monthly') ? 'selected' : ''; ?>>Monthly</option>
                <option value="hourly" <?php echo ($user['salary_type'] == 'hourly') ? 'selected' : ''; ?>>Hourly</option>
            </select>
        </div>
        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2" for="salary_rate">
                Salary Rate
            </label>
            <input class="shadow-sm appearance-none border rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" id="salary_rate" name="salary_rate" type="number" step="0.01" value="<?php echo htmlspecialchars($user['salary_rate']); ?>">
        </div>
        <div class="mb-4">
