...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_deduction'])) {
        ...
        try {
            $stmt = $db->prepare("INSERT INTO deductions (user_id, amount, reason, date) VALUES (?, ?, ?, ?)");
            if ($stmt->execute([$user_id, $amount, $reason, $date])) {
                set_flash_message('success', 'Deduction added successfully!');
            } else {
                set_flash_message('error', 'Failed to add deduction.');
            }
...
// Fetch users and deductions
$users_stmt = $db->query("SELECT id, name FROM users WHERE role_id = (SELECT id FROM roles WHERE name = 'user')");
$users = $users_stmt->fetchAll(PDO::FETCH_ASSOC);

$deductions_stmt = $db->query("SELECT deductions.*, users.name as user_name FROM deductions JOIN users ON deductions.user_id = users.id");
...
<div class="container mx-auto px-4 sm:px-8">
    <div class="py-8">
        <div>
            <h2 class="text-2xl font-semibold leading-tight">Deductions</h2>
        </div>
            ...
            <div class="flex flex-row mb-1 sm:mb-0">
                <div class="relative">
                    <form method="POST">
                        ...
                        <button type="submit" name="add_deduction" class="px-4 py-2 bg-blue-500 text-white rounded-r">Add Deduction</button>
                    </form>
                </div>
            </div>
        </div>
                        ...
                        <?php foreach ($deductions as $deduction): ?>
                            <tr>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap"><?php echo htmlspecialchars($deduction['user_name']); ?></p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap"><?php echo htmlspecialchars($deduction['amount']); ?></p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap"><?php echo htmlspecialchars($deduction['reason']); ?></p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap"><?php echo htmlspecialchars($deduction['date']); ?></p>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
